...
function front_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			...
			if($curr_page==1) {
				/*$multipage .= ' <span>1 C</span>';*/
				$multipage .= ' <li class="active"><a href="javascript:;">1<span class="sr-only">(current)</span></a></li>';
			} elseif($curr_page>6 && $more) {
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= ' <li class="active"><a href="javascript:;">'.$i.'<span class="sr-only">(current)</span></a></li>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
			...
			$multipage .= ' <li class="active"><a href="javascript:;"><span>'.$pages.'<span class="sr-only">(current)</span></a> <li><a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a></li>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
		}
	}
